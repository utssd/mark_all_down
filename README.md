...
- Building on that, a detected **GPU-process crash now auto-recovers** instead of waiting for a manual banner click. The crash event starts a **5-second countdown** in the banner (with **Reload now** and **Cancel** buttons), then reloads — and since shells survive a reload, the terminals come back automatically. It only counts down while the **Terminal tab is active** (a crash on another tab shows a passive banner and defers the countdown until you open the Terminal tab, so editing is never interrupted), and a 30-second anti-storm guard stops a repeatedly-crashing GPU from reload-looping.
